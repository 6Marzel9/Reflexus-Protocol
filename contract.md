# Reflexus Protocol (UTR) - Smart Contract Source Code
...
- **Burning Limit:** 250,000,000 UTR (20% of total supply)
- **Minimum Purchase:** 0.0001 BNB
- **Base Price:** 0.0001 BNB per token

### Fee Structure (Immutable Constants in BPS - Basis Points)
- **Buy Fees:** 5 BPS dev + 10 BPS reserve + 10 BPS reflection
- **Refund Fees:** 5 BPS dev + 5 BPS reflection + 7.5 BPS burn (if under limit) + 7.5-15 BPS reserve
- **Transfer Fees:** 5 BPS dev + 10 BPS reflection + 10 BPS reserve
- **DEX Swap Fees:** 5 BPS dev + 10 BPS reflection + 10 BPS reserve

### Security Features Implemented
1. ✅ **ReentrancyGuard** - Protects buy() and refund() functions
2. ✅ **OpenZeppelin Libraries** - ERC20, ERC20Permit, Ownable2Step
3. ✅ **Immutable Fees** - All fee percentages are immutable constants
4. ✅ **Burn Cap Enforcement** - Maximum 20% of supply can be burned
5. ✅ **Custom Errors** - Gas-efficient error handling
6. ✅ **SafeMath via Math.mulDiv** - Prevents overflow/underflow
7. ✅ **Event Emissions** - All state changes emit events
8. ✅ **Access Control** - Owner-only functions for dev address and fee exclusions

### Owner-Only Functions (Disabled After Renouncement)
- `setDevAddress(address)` - Change dev fee recipient
- `excludeFromFee(address, bool)` - Add/remove fee exclusions

### Public View Functions
- `balanceOf(address)` - Get token balance
- `totalSupply()` - Get current circulating supply (minus burned)
- `calculateUtrForNative(uint256)` - Calculate UTR for BNB amount
- `pendingDividends(address)` - View unclaimed dividends
- `getTotalDividendsDistributed()` - Total dividends distributed
- `getMagnifiedDividendPerShare()` - Current dividend tracking value
- `getCirculatingSupplyPublic()` - Get circulating supply
- `getUserDividendInfo(address)` - Complete user dividend data

### Core Interaction Functions
- `buy()` / `receive()` - Purchase UTR with BNB
- Transfer to contract address - Refund UTR for BNB
- `claimDividends()` - Claim accumulated reflection rewards
- Standard ERC20 functions (transfer, approve, etc.)

## Verification
This contract is verified on BscScan at: `https://bscscan.com/token/0x649e91d212CcA1e21D4f991a2580f175b95924EB#code`

All contract code above matches the deployed and verified contract on Binance Smart Chain.